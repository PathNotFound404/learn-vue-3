...
const stopWatch = watch(number, (newValue, oldValue) => {
  ...
  // Stop watching when number reaches 5
  if (newValue >= 5) {
    stopWatch()
  }
}, { immediate: true }) 
...
watch(
  () => wizard1.value.name, // Cant directly watch a property of a ref(object), need to use a getter function.
  (newValue, oldValue) => {
    console.log(
...
watch(
  ...
  { deep: true } // if just the core or name was changed, the watcher will not be triggered without deep: true, because the reference of wand is not changed.
)
